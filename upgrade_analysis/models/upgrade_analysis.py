...
class UpgradeAnalysis(models.Model):
    ...
    def _remote_series(self):
        ...
        The remote reports whatever odoo.release.version holds, which is a bare
        "19.0" only for a source checkout. Packaged builds append their build
        date -- the official docker images report "19.0-20260908" -- and deb
        installs add a suffix too, so the string cannot be handed to float().
        """
        self.ensure_one()
    ...
    def generate_module_coverage_file(self, no_changes_modules):
        ...
        # connection.version is the remote's odoo.release.version, which is a
        # bare series only for a source checkout; packaged builds append a build
        # date and would leak it into the file name and the document title.
